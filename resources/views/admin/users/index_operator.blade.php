...
@section('title', 'Users - Operator')

@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Operator Users</h1>
            <p class="text-slate-500 mt-1">Manage operators who handle daily lending tasks.</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.users.export_operators') }}"
    ...
    @if (session('generated_password'))
        <script>
            alert("The new password is: {{ session('generated_password') }}");
        </script>
        <div class="mb-6 p-4 bg-amber-50 border border-amber-100 text-amber-800 rounded-2xl flex items-center justify-between">
            <div class="flex items-center">
                ...
                <p class="text-sm font-medium">Reset Password: <span class="font-bold font-mono px-2 py-0.5 bg-white rounded border border-amber-200">{{ session('generated_password') }}</span></p>
            </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach ($users as $user)
                        <tr class="hover:bg-slate-50/50 transition-colors duration-150">
                            ...
                            <td class="py-4 px-6">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-100">
                                    Operator
                                </span>
                            </td>
                            <td class="py-4 px-6 text-right">
                                <div class="flex items-center justify-end gap-3">
                                    <form method="POST" action="{{ route('admin.users.reset_password', $user) }}" class="inline">
                                        @csrf
                                        <button type="submit"
                                            class="inline-flex items-center px-3 py-1.5 text-xs font-bold text-amber-600 bg-amber-50 border border-amber-100 rounded-lg hover:bg-amber-100 transition-colors"
                                            onclick="return confirm('Reset this user\'s password to default?')" title="Reset Password">
